...
public:
	SMTP_Client(const char* serverIP,int portno)
	{
	...
	void run_4a()
	{
		...
		establishConnection_4a();

		createANDsendMailContent_4a();

		terminateConnection_4a();

		close(sockfd);

	}

	void establishConnection_4a()
	{
		int write_count,read_count;
		char packet_str[MAX_SIZE];

		memset(packet_str,'\0', sizeof(packet_str));
		read_count=read(sockfd,(char*)&packet_str,sizeof(packet_str));
		if(read_count<0) error("Error in reading reply from Server !");
		else cout << "Server Sent :\n" << packet_str << endl;

		memset(packet_str,'\0', sizeof(packet_str));
		strcpy(packet_str,"HELO garudaserver.com\r\n");
		write_count = write(sockfd,(char*)&packet_str,strlen(packet_str));
		if(write_count<0) error("Error in writing HELO to Server !");
		else cout << "Sent to Server :\n" << packet_str << endl;
	...
	void createANDsendMailContent_4a()
	{
		int write_count,read_count;
		char packet_str[MAX_SIZE];
		char buf[MAX_SIZE];
		...
		/********************************************************************/
		memset(packet_str,'\0', sizeof(packet_str));
		strcpy(packet_str,"MAIL FROM: kumrayush@garudaserver.com\r\n");
		write_count = write(sockfd,(char*)&packet_str,strlen(packet_str));
		...
		/********************************************************************/
		memset(packet_str,'\0', sizeof(packet_str));
		strcpy(packet_str,"RCPT TO: testuser@garudaserver.com\r\n");
		write_count = write(sockfd,(char*)&packet_str,strlen(packet_str));
		...
		memset(packet_str,'\0', sizeof(packet_str));
		sprintf(packet_str,"FROM : %s\r\n",buf);

		write_count = write(sockfd,(char*)&packet_str,strlen(packet_str));
		if(write_count<0) error("Error in writing header FROM to Server !");
		...
		memset(packet_str,'\0', sizeof(packet_str));
		sprintf(packet_str,"TO : %s\r\n",buf);

		write_count = write(sockfd,(char*)&packet_str,strlen(packet_str));
		if(write_count<0) error("Error in writing header TO to Server !");

		/********************************************************************/
		time_t now = time(0);
		memset(packet_str,'\0', sizeof(packet_str));
		sprintf(packet_str,"DATE : %s\r\n",asctime(localtime(&now)));

		write_count = write(sockfd,(char*)&packet_str,strlen(packet_str));
		if(write_count<0) error("Error in writing header DATE to Server !");
		else cout << packet_str;
		/********************************************************************/
		...
		memset(packet_str,'\0', sizeof(packet_str));
		sprintf(packet_str,"SUBJECT : %s\r\n",buf);

		write_count = write(sockfd,(char*)&packet_str,strlen(packet_str));
		if(write_count<0) error("Error in writing header SUBJECT to Server !");
		...
		do
		{
				...
				if(strcmp(buf,".")==0)
					...
					sprintf(packet_str,"%s\r\n",buf);

				write_count = write(sockfd,(char*)&packet_str,strlen(packet_str));
				if(write_count<0) error("Error in writing body line to Server !");
	...
	void terminateConnection_4a()
	{
		int write_count,read_count;
		char packet_str[MAX_SIZE];

		memset(packet_str,'\0', sizeof(packet_str));
		strcpy(packet_str,"QUIT\r\n");
		write_count = write(sockfd,(char*)&packet_str,strlen(packet_str));
