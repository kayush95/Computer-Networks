...
public:
	POP_Client(const char* serverIP,int portno)
	{
...
*/
	...
	bool authorize_4a()
	{
		...
		sprintf(packet_str,"USER: %s\r\n",buf);

		write_count = write(sockfd,(char*)&packet_str,strlen(packet_str));
		if(write_count<0) error("Error in writing header USER to Server !");
		...
		memset(packet_str,'\0', sizeof(packet_str));
		sprintf(packet_str,"PASS: %s\r\n",buf);

		write_count = write(sockfd,(char*)&packet_str,strlen(packet_str));
		if(write_count<0) error("Error in writing header PASS to Server !");
		...
		memset(packet_str,'\0', sizeof(packet_str));
		strcpy(packet_str,"LIST\r\n");

		write_count = write(sockfd,(char*)&packet_str,strlen(packet_str));
		if(write_count<0) error("Error in writing header LIST to Server !");
